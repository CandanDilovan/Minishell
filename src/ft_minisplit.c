...
/*   Updated: 2023/08/04 23:48:42 by dilovancand      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	is_quote(char *tab, char *s, char c, char letters)
{
	char	quote;
	int		d;
...
static char	**into_tab(char **tab, char const *s, char c)
{
	...
	while (s[letters])
	{
		...
		{
			if (is_quote(tab[b], s, c, letters) == -1)
				return (free_all(tab, (b - 1)));
			letters = is_quote(tab[b++], s, c, letters);
		}
		else if (issep(s[letters], c) == 0)
		{
			if (no_quote(tab[b], s, c, letters) == -1)
				return (free_all(tab, (b - 1)));
			letters = no_quote(tab[b++], s, c, letters);
		}
		else
			letters++;
	}
...
char	**ft_minisplit(char const *s, char c)
{
	...
	if (!s)
		return (NULL);
	tab_nb = ft_count_tab(s, c, a, tab_nb);
